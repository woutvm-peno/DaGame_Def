 ...
 */
public class SensorActMF extends Activity implements SensorEventListener {

    private SensorManager sensorManager;

    public minigame3 mf_class;


    public SensorActMF(minigame3 mf) {
        mf_class = mf;
    }
    ...
    public double magnetic_field;
    public String field_magnetic;
    public double max_mf = 0.0;
    public String mf_max;
    void Verwerk_Magn(float x, float y, float z) {

        magnetic_field = Math.sqrt(x*x+y*y+z*z);
        DecimalFormat numberFormat = new DecimalFormat("#.00");
        field_magnetic = numberFormat.format(magnetic_field);
        mf_class.update(field_magnetic);
        if (magnetic_field > max_mf) {
            max_mf = magnetic_field;
        }
        mf_max = numberFormat.format(max_mf);
        mf_class.update_two(mf_max);
